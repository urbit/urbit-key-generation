...
test('child seed from seed', async () => {
  let res = await kg.childSeedFromSeed({
    seed: 'some seed',
    type: 'type',
    revision: 0
  });
  ...
  //
  res = await kg.childSeedFromSeed({
    seed: 'some seed!',
    type: 'type',
    revision: 0
  });
  ...
  //
  let res2 = await kg.childSeedFromSeed({
    seed: 'some seed',
    type: 'type',
    revision: 0,
    ship: 2,
...
test('wallet from seed', async () => {
  let res = await kg.walletFromSeed('some seed');
  expect(res).toBe({
  ...
  //
  let res2 = await kg.walletFromSeed('some seed', '');
  expect(res2).toBe(res);
  //
  res = await kg.walletFromSeed('some seed', 'pass');
  expect(res).toBe({
...
test('child node from seed', async () => {
  let res = await kg.childNodeFromSeed({
    seed: 'some seed',
    type: 'type',
    revision: 0
  });
  expect(res.meta).toBe({type: 'type', revision: 0, ship: null});
  expect(res.seed).toBe('b150354a72552c9efd');
  expect(res.keys).toBe({
  ...
  //
  res = await kg.childNodeFromSeed({
    seed: 'some seed',
    type: 'type',
    revision: 0,
    ship: 2,
  ...
  expect(res.meta).toBe({type: 'type', revision: 0, ship: 2});
...
test('urbit keys from seed', async () => {
  ...
  let res = kg.urbitKeysFromSeed(seed, Buffer.from(''));
  expect(res.crypt).toBe({
  ...
  //
  let res = kg.urbitKeysFromSeed(seed, Buffer.from('pass'));
  expect(res.crypt).toBe({
